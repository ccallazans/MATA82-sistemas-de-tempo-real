...
def criar_grafico_gantt(resultado, tarefas, eventos):
    ...
    for i, tarefa in enumerate(tarefas):
        execucoes = [r for r in resultado if r[0] == tarefa.id]
        for id, inicio, fim in execucoes:
            ax1.broken_barh([(inicio, fim - inicio)], (i, 0.9), facecolors=cores[i])
            if fim - inicio > 0.5:
                ax1.text(inicio + (fim - inicio)/2, i + 0.5, f'T{id}', ha='center', va='center')
